...
CsvDriver::CsvDriver(const std::string sFileAccel,
...
void CsvDriver::_ThreadCaptureFunc() {
  // Wait until something is listening to start polling.
  while (!m_bShouldRun) {
...
inline bool CsvDriver::_GetNextTime(double& dNextTime, double& dNextTimePPS) {
  std::string sValue;
